...
trait CertificateTrait
{
    ...
    public function generateCertificate($template, $enroll, $serialNumber)
    {
        try {
            ...
            $templateDesign = 'frontend/default/certificate.png'; //showImage(@$template->image->original, 'frontend/default/certificate.png');
    ...
    {
        $logo_path = showImage(setting('favicon.ico'), 'favicon.ico');
        $extension = pathinfo($logo_path, PATHINFO_EXTENSION);
        $image = Image::make(asset('/backend/uploads/default-images/favicon.ico'));
    ...
    public function createCertificate($certificate_name, $course, $instructor, $template, $serialNumber)
    {
        try {
            ...
            $imageContent = file_get_contents($template);
            // Create an image resource from the content
            $certificate = imagecreatefromstring($imageContent);
            ...
            $qrCodeImage = imagecreatefrompng($qrCodeData);
